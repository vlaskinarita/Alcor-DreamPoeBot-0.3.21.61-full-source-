...
public class Item : RemoteMemoryObject
{
	...
	public int ChargesPerUse
	{
		get
		{
			if (!nullable_59.HasValue)
			{
				...
				else if (!Stats.TryGetValue(StatTypeGGG.LocalChargesUsedPosPct, out value))
				{
	...
	public float HealthRecoveredPerSecond
	{
		get
		{
			if (!nullable_53.HasValue)
			{
				...
				else if (!IsInstantRecovery && !RecoveryTime.TotalSeconds.Equals(0.0))
				{
	...
	public int GetSocketIndexOfGem(Item gem)
	{
		...
		for (int i = 0; i < 6; i++)
		{
			Item item = socketsComponent.Gems[i];
