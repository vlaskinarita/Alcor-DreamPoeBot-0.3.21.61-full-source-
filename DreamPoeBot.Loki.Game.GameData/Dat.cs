...
public static class Dat
{
	...
	private sealed class _003Cget_BaseItemTypes_003Ed__28 : IEnumerable<DatBaseItemTypeWrapper>, IEnumerator<DatBaseItemTypeWrapper>, IDisposable, IEnumerator, IEnumerable
	{
		...
		void IDisposable.Dispose()
		{
			...
			while (true)
			{
				...
				{
				default:
					switch ((num2 = (num2 * 2745811823u) ^ 0x11A852B8u ^ 0x29126600u) % 4u)
					{
					case 0u:
					case 3u:
						goto end_IL_003b;
		...
		private bool MoveNext()
		{
			try
			{
				...
				{
					KeyValuePair<string, BaseItemType> current = _003C_003E7__wrap3.Current;
	...
	public static IEnumerable<DatMinimapIconWrapper> MinimapIcons
	{
		get
		{
			...
			using Dictionary<long, DatMinimapIconWrapper>.Enumerator enumerator = dictionary_MinimapIconWrapper.GetEnumerator();
	...
	public static IEnumerable<DatPassiveSkillWrapper> PassiveSkills
	{
		get
		{
			if (!dictionary_IdToPassiveSkillWrapper.Any())
			{
				...
				for (int i = 0; i < struct131s.Count; i++)
				{
					yield return new DatPassiveSkillWrapper(struct131s[i], i + 1);
				}
				yield break;
			}
			foreach (KeyValuePair<int, DatPassiveSkillWrapper> item in dictionary_IdToPassiveSkillWrapper)
			{
				yield return item.Value;
			}
		}
	...
	public static IEnumerable<DatStatWrapper> Stats
	{
		get
		{
			...
			{
				using (Dictionary<long, DatStatWrapper>.Enumerator enumerator = dictionary_StatWrapper.GetEnumerator())
				{
					if (enumerator.MoveNext())
					{
						yield return enumerator.Current.Value;
											...
											while (true)
											{
												...
												if (!(fieldInfo.Name == "QuestId"))
												{
													while (true)
													{
														...
														int num5 = (int)((num4 * 675973953) ^ 0x30B2D4B0);
														goto IL_0190;
														IL_02fe:
														...
														while (true)
														{
															switch ((num4 = (uint)num5 ^ 0xAEBACE88u) % 45u)
															{
															case 19u:
																num5 = (int)((num4 * 762181785) ^ 0x7D3CBEDC);
																continue;
															case 7u:
																num5 = ((int)num4 * -1347149085) ^ 0x7513BB6A;
																continue;
															case 1u:
																break;
															default:
														...
														num5 = ((int)num4 * -1294598341) ^ 0x41F98BC3;
														goto IL_0190;
														IL_0352:
	...
	internal static void smethod_14()
	{
		...
		foreach (KeyValuePair<string, BaseItemType> item in contents)
		{
			try
			{
				...
				{
					if (!text.Contains("_Microtransaction") && !text.Contains("_Pets_") && !text.Contains("_Hideout_") && !text.Contains("_Heist_") && !text.Contains("Royale") && !text.Contains("_Currency_RandomFossilOutcome"))
					{
