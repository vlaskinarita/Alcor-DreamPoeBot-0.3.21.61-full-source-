	...
	private static Vector2i PointOnRect(Vector2i dest, RectangleF rectangle)
	{
		...
		if (!rectangle.Contains(result.X, result.Y))
		{
			...
			while (enumerator.MoveNext())
			{
				...
				case side.Left:
				{
					if (!Intersects(a, result.ToVector2(), b, vector2, out var intersection4))
					{
						break;
					}
				...
				case side.Righ:
				{
					if (!Intersects(a, result.ToVector2(), vector, b2, out var intersection2))
					{
						break;
					}
				...
				case side.Top:
				{
					if (Intersects(a, result.ToVector2(), b, vector, out var intersection3))
					{
				...
				case side.Bottom:
				{
					if (Intersects(a, result.ToVector2(), vector2, b2, out var intersection))
					{
