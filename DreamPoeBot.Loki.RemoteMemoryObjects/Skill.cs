							...
							while (true)
							{
								...
								while (true)
								{
									...
									while (true)
									{
										...
										switch ((num = (num * 3967900429u) ^ 0x630BD2FAu ^ 0xBC3FD133u) % 19u)
										{
										case 8u:
											break;
										case 7u:
	...
	public bool CanUseEx(out string error, bool ignoreOnSkillBar = false, bool checkCharges = true)
	{
		...
		if (Struct135_0.byte_6CanBeUsedWithWeapon == 1)
		{
			if (Struct135_0.byte_7CannotBeUsed == 0)
			{
				...
				if (!RequiresReservedPower)
				{
					if (!RequiresPercentPower)
					{
						...
						if (GetStatFromAll(StatTypeGGG.VirtualEnergyShieldProtectsMana) == 0 && me.GetStat(StatTypeGGG.VirtualEnergyShieldProtectsMana) == 0)
						{
							...
							float num2 = ((costType == SkillCostType.Life) ? LifeCost : ManaCost);
							if ((float)num < num2)
							{
						...
						bool flag;
						if (!(flag = (costType == SkillCostType.Life && Cost < me.Health) || Cost < me.EnergyShield + me.Mana))
						{
							error = $"We cannot cast [{Name}] because Eldrich Battery affect this skill and costMet = {flag} [costType = {costType}].";
						}
						return flag;
					}
				...
				float num10 = ((costType3 == SkillCostType.Life) ? LifeReservation : ManaReservation);
				if ((float)num9 >= num10)
				{
					return true;
				}
